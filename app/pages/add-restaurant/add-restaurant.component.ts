...
if (!/^[A-Za-z\s]{3,}$/.test(this.restaurant.city)) {
  alert('City name must contain only letters, be at least 3 characters long.');
  return false;
}
if (!/^[-+]?([1-8]?\d(\.\d+)?|90(\.0+)?)\s*,\s*[-+]?((1[0-7]\d(\.\d+)?|180(\.0+)?)|\d{1,2}(\.\d+)?)$/.test(this.restaurant.geoLocation)) {
  alert('Geo Location must be in latitude,longitude format (e.g., 12.9716,77.5946).');
  return false;
}
